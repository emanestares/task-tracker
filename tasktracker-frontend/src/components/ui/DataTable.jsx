...
export default function DataTable({
  columns, data, loading, emptyTitle = 'No data yet',
  emptyDescription, sortKey, sortDir, onSort, rowKey = 'id',
}) {
...
function TableSkeleton({ columns }) {
  return (
    <div className="overflow-hidden rounded-xl" style={{ border: '1px solid var(--border-primary)' }}>
      <div className="px-4 py-3" style={{ backgroundColor: 'var(--bg-tertiary)', borderBottom: '1px solid var(--border-primary)' }}>
      ...
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="px-4 py-4 flex gap-6" style={{ borderBottom: '1px solid var(--border-primary)' }}>
          {columns.map((col, j) => (
            <div key={col.key} className="skeleton h-4 rounded-md"
