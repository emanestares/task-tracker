...
// ── StatCard ──────────────────────────────────────────────────────────────────
export function StatCard({ label, value, icon: Icon, loading, color = 'var(--accent-primary)', trend }) {
  return (
    <div className="card group">
      <div className="flex items-start justify-between mb-4">
        ...
        {trend !== undefined && (
          <span
            ...
            style={{ backgroundColor: trend >= 0 ? '#ecfdf5' : '#fef2f2', color: trend >= 0 ? '#059669' : '#ef4444' }}
          >
            {trend >= 0 ? '+' : ''}{trend}%
          </span>
        )}
      ...
      {loading ? (
        ...
        <p className="text-3xl font-bold leading-none mb-1" style={{ color: 'var(--text-primary)', fontFamily: 'Inter Tight, Inter, sans-serif' }}>
          {value ?? '—'}
        </p>
      )}
...
export function StatusBadge({ status }) {
  const map = {
    TODO:        { label: 'To Do',       cls: 'badge-todo',        dot: '#94a3b8' },
  ...
  const { label, cls, dot } = map[status] || { label: status, cls: 'badge-todo', dot: '#94a3b8' }
  return (
...
export function Pagination({ page, totalPages, onChange }) {
  ...
  return (
    <div className="flex items-center justify-center gap-1.5 mt-5">
      <PBtn onClick={() => onChange(page - 1)} disabled={page === 1} label="←" />
      {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
        <PBtn key={p} onClick={() => onChange(p)} active={p === page} label={String(p)} />
      ))}
      <PBtn onClick={() => onChange(page + 1)} disabled={page === totalPages} label="→" />
    </div>
  )
}
function PBtn({ onClick, disabled, active, label }) {
  return (
    <button
      ...
      style={{
        background: active ? 'linear-gradient(135deg,#2563eb,#1d4ed8)' : 'var(--bg-secondary)',
        color: active ? '#fff' : 'var(--text-secondary)',
        border: active ? 'none' : '1px solid var(--border-primary)',
        boxShadow: active ? '0 2px 8px rgba(37,99,235,0.3)' : '0 1px 2px rgba(0,0,0,0.04)',
