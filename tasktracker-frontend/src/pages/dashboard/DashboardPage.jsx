        ...
        {segments.map((seg) => {
          ...
          return (
            <div
              ...
              style={{ height: 4, background: 'var(--border-primary)' }}
            >
              <div
  ...
  const urgentTasks = useMemo(
    () =>
      tasks
        .filter(
          (t) =>
            t.priority === TASK_PRIORITY.HIGH && t.status !== TASK_STATUS.DONE
        )
        .slice(0, 3),
    [tasks]
  );
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {STAT_CARDS(stats).map(
          ({ label, value, icon: Icon, gradient, iconColor, border }) => (
            <div
              key={label}
          ...
          <div>
            {recent.map((task, i) => (
              ...
              >
                ...
                <StatusBadge status={task.status} />
                <span
                  className="text-xs hidden sm:block flex-shrink-0 w-16 text-right"
                  style={{ color: 'var(--text-muted)' }}
                >
