...
function DonutChart({ segments, total }) {
  ...
  const arcs = segments.reduce(
    (acc, seg) => {
      ...
      acc.arcs.push({
        ...
        dashOffset: -previous * circumference,
      });
      acc.offset += pct;
      return acc;
    },
  ...
  const donePct =
    total > 0
      ? Math.round(
          ((segments.find((s) => s.key === 'DONE')?.value || 0) / total) * 100
        )
      : 0;
  ...
  const stats = useMemo(() => {
    ...
    const byPriority = ['HIGH', 'MEDIUM', 'LOW'].map((k) => ({
      ...
      inProgress: tasks.filter(
        (t) => t.priority === k && t.status === 'IN_PROGRESS'
      ).length,
  ...
  const prioritySegments = [
    {
      key: 'HIGH',
      label: 'High',
      value: stats.byPriority[0]?.total ?? 0,
      color: '#ef4444',
    },
    {
      key: 'MEDIUM',
      label: 'Medium',
      value: stats.byPriority[1]?.total ?? 0,
      color: '#f59e0b',
    },
    {
      key: 'LOW',
      label: 'Low',
      value: stats.byPriority[2]?.total ?? 0,
      color: '#10b981',
    },
  ];
        ...
        <div className="card" style={{ padding: 24 }}>
          ...
          {stats.total === 0 ? (
            ...
            <DonutChart
              ...
              total={prioritySegments.reduce((s, seg) => s + seg.value, 0)}
            />
          )}
        </div>
      </div>
        ...
        >
          <div>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: 0 }}>
              To Do / In Progress / Done per priority level
            </p>
          </div>
          <div
            style={{
...
export default function TasksPage() {
  ...
  const filtered = useMemo(() => {
    ...
    r = [...r].sort((a, b) => {
      ...
      return (
        direction * (new Date(b.createdAt || 0) - new Date(a.createdAt || 0))
      );
    });
    return r;
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  fontWeight: 600,
                  backgroundColor:
                    view === id ? 'var(--accent-primary)' : 'transparent',
                  color: view === id ? '#fff' : 'var(--text-secondary)',
                  boxShadow:
                    view === id ? '0 1px 4px rgba(37,99,235,.35)' : 'none',
                  transition: 'all .15s',
                }}
              >
                <Icon size={15} />
            ...
            >
              {[1, 2, 3, 4, 5].map((i) => (
                ...
                >
                  <div
                    className="skeleton"
                    style={{ height: 16, flex: 1, borderRadius: 8 }}
                  />
                  <div
                    className="skeleton"
                    style={{ height: 32, width: 96, borderRadius: 999 }}
                  />
                  <div
                    className="skeleton"
                  ...
                  <div
                    className="skeleton"
                    style={{ height: 32, width: 64, borderRadius: 8 }}
                  />
                </div>
              ))}
            </div>
